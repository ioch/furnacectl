...
class Arduino:
    ...
    def line_status(self, line):
        ...
        t = float(t_ms) / 1000.0
        power = float(pwm)/20.0
        temp_outside = float(temp_outside)
        temp_inside = float(temp_inside)
        return Status(t, local_time(), power, float(setpoint), temp_outside, temp_inside)
    ...
    def power(self, power):
        assert(self.started.is_set())
        pwm = bytes(power)
        command = struct.pack( '%isc' % (len(pwm)), pwm, b'\r')
    ...
    #~ def setpoint(self, setpoint):
        #~ assert(self.started.is_set())
        #~ x = int(setpoint)
        #~ command = struct.pack('pB', b'point ', x)
        #~ self.command.put(command)
...
class Krosnis:
    def __init__(self, root, experiment):
        ...
        self.set_power = tk.Button(self.toolbar, text='Send', command=self.set_power)
    ...
    def set_power(self, event=None):
        self.arduino.power = bytes(self.power_val.get(), 'utf8')
